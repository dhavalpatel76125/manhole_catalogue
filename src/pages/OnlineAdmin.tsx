...
export default function OnlineAdmin() {
  ...
  const filtered = useMemo(() => products.filter(p => `${p.title} ${p.product_code || ''} ${p.category || ''}`.toLowerCase().includes(search.trim().toLowerCase())), [products, search])
  ...
  return <>
    ...
    <main className="admin-shell"><div className="admin-top"><div><p className="eyebrow"><span/> LIVE PRODUCT MANAGEMENT</p><h1>Catalogue admin<span>.</span></h1><p>Manage products, categories, capacities, sizes and measurements. Saved changes appear online.</p></div><div className="admin-actions"><button className="secondary-button" disabled={busy} onClick={async () => { setBusy(true); try { await refresh(); setError('') } catch (e) { setError(errorMessage(e)) } finally { setBusy(false) } }}><RefreshCw size={16}/>Refresh</button><a className="secondary-button" href="/" target="_blank" rel="noopener noreferrer">View catalogue</a></div></div>
      {error && <div className="notice error-note" role="alert">{error}</div>}
      <div className="admin-toolbar"><div className="search-field"><Search size={19}/><input aria-label="Search admin products" placeholder="Search by name, code or category..." maxLength={160} value={search} onChange={e => { setSearch(e.target.value); setPage(1) }}/></div><span className="admin-count">{products.length} products · {products.filter(p => p.is_active).length} active</span><button className="primary-button" disabled={busy || !catalogue} onClick={() => setEditing('new')}><Plus size={17}/>Add product</button></div>
      {!catalogue ? <div className="empty-state" role="status">Loading products…</div> : !filtered.length ? <div className="empty-state"><Search size={35}/><h2>{search ? 'No products found' : 'Your catalogue starts here'}</h2><p>{search ? 'Try another name or product code.' : 'Add your first product and it will appear in your online catalogue.'}</p></div> : <>
        <div className="table-wrap"><table><thead><tr><th>Product</th><th>Visibility</th><th>Order</th><th>Actions</th></tr></thead><tbody>{filtered.slice((page - 1) * PAGE_SIZE, page * PAGE_SIZE).map(product => { const index = products.findIndex(p => p.id === product.id); return <tr key={product.id}>
          <td><div className="table-product"><img src={product.image_url} alt={product.title}/><div><strong>{product.title}</strong>{product.category && <span className="category-label">{product.category}</span>}<small>{[product.product_code, product.load_capacity, product.size].filter(Boolean).join(' · ') || 'Details not specified'}</small></div></div></td>
    ...
    {editing && <ProductEditor products={products} product={editing === 'new' ? null : editing} imageUrl={editing === 'new' ? undefined : editing.image_url} onSave={save} onClose={() => setEditing(null)} saveLabel="Save product"/>}
