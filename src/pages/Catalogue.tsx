...
export default function Catalogue() {
  ...
  const { products, total, loading, error, refresh, capacities, sizes } = useProducts(search, page, false, capacity, size, category)
  const hasFilters = Boolean(search || capacity || size || category)
  const clearFilters = () => { setSearch(''); setCapacity(''); setSize(''); setCategory(''); setPage(1) }
  ...
  return <>
    ...
    <main className="catalogue-shell" id="catalogue" ref={catalogueTop} aria-labelledby="catalogue-title">
      ...
      <div className="catalogue-toolbar"><div className="catalogue-filters">
        <label className="filter-field">Category<select aria-label="Category" value={category} onChange={e => { setCategory(e.target.value); setCapacity(''); setSize(''); setPage(1) }}><option value="">All categories</option>{PRODUCT_CATEGORIES.map(value => <option key={value} value={value}>{value}</option>)}</select></label>
      ...
      {error ? <div className="empty-state" role="alert"><PackageSearch size={38}/><h2>We couldn’t load the catalogue</h2><p>Please try again in a moment.</p><button className="secondary-button" onClick={refresh}><RotateCw size={16}/>Try again</button></div> : loading ? <div className="product-grid" aria-label="Loading products" aria-busy="true">{Array.from({ length: 12 }, (_, i) => <div className="skeleton-card" key={i}><div/><span/><span/><span/></div>)}</div> : !products.length ? <div className="empty-state"><PackageSearch size={40}/><h2>{hasFilters ? 'No products found' : 'Our catalogue is taking shape'}</h2><p>{hasFilters ? 'Try another category, load capacity, size or product name.' : 'New products will appear here soon. Contact us for current availability.'}</p>{hasFilters && <button className="secondary-button" onClick={clearFilters}>Clear filters</button>}</div> : <div className="product-grid">{products.map(product => <ProductCard key={`${product.id}-${product.image_url}`} product={product} quantity={quantities[product.id] || 1} onQuantity={q => setQuantities(old => ({ ...old, [product.id]: q }))}/>)}</div>}
