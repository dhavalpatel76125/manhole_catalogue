...
export function Pagination({ page, pages, onChange }: { page: number; pages: number; onChange: (page: number) => void }) {
  ...
  return <nav className="pagination" aria-label="Catalogue pages">
    <button className="page-edge" onClick={() => onChange(page - 1)} disabled={page === 1}><ChevronLeft size={16}/><span>Previous</span></button>
    ...
    <button className="page-edge" onClick={() => onChange(page + 1)} disabled={page === pages}><span>Next</span><ChevronRight size={16}/></button>
  </nav>
}
