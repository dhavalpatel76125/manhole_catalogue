...
describe('Fibro catalogue data flow', () => {
 ...
 it('sorts written numbers across units, rectangles and diameters without changing source order', () => {
  ...
  const expected = ['12 × 12 inch', '12 × 18 inch', '24 × 24 inch', '30 × 30 inch', '300 × 300 mm', '600 mm diameter', 'Custom', null]
  ...
  expect(compareSizes('24 x 18 INCH', '18 × 24 inch')).toBeGreaterThan(0)
  expect(compareSizes('12 inch', '304.8 mm')).toBeLessThan(0)
 })
 it('keeps equal-sized variants in admin order and places unknown dimensions last', () => {
  const rows = [{...base,id:'unknown',size:null},{...base,id:'first'},{...base,id:'second',size:'24 × 24 mm'}]
  expect(matchingProducts(rows,'').map(p=>p.id)).toEqual(['first','second','unknown'])
  expect(compareSizes('300x300', '5 inch')).toBeGreaterThan(0)
 })
 it('orders the requested 2.5-ton sizes as 36x36, 42x42, then 1000x1000', () => {
  const rows = ['1000 × 1000 mm', '42 × 42 inch', '36 × 36 inch'].map(size => ({...base,size,load_capacity:'2.5 ton'}))
  const expected = ['36 × 36 inch', '42 × 42 inch', '1000 × 1000 mm']
  expect(matchingProducts(rows,'','2.5 ton').map(p=>p.size)).toEqual(expected)
  expect(productOptions(rows,'size')).toEqual(expected)
  expect(compareSizes('36x36','42x42')).toBeLessThan(0)
 })
